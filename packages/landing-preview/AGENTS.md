...
- Depends on `@workspace/prompt-panel` for `ElementAttachmentInput` and `ScreenshotMediaType`, on catalog `@zumer/snapdom` for rasterization, and on catalog `morphdom` for preview DOM morphing; `react`/`react-dom` are peer deps.
