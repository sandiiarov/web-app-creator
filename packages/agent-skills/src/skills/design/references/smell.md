...
**Important:** Smell never applies fixes itself — it names the tells and points to the discipline that fixes each. Findings live in your working answer; no files are written.
...
**MAX_SCORE = 10.** Use `/10` as the denominator when you summarize the score. When zero tells are found, the score is `10/10` and the verdict is CLEAN. Never output `0/10` to mean "no smells detected."

Score each of the ten odors: `1` if absent, `0` if detected.
...
- Findings are stated in the working answer (no files written)

STRICT RULE — NEVER BREAK THIS
Run smell in memory. Never write a report, summary, or any file to disk —
findings live in your working answer and drive the fixes you make.
