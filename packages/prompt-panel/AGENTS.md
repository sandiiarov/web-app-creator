...
- `src/domain.ts`: landing conversation domain model — model options (text `LANDING_MODEL_OPTIONS`, plus `LANDING_VISION_MODEL_OPTIONS` and `LANDING_IMAGE_MODEL_OPTIONS`), the per-category `LandingModels` selection + `DEFAULT_LANDING_MODELS` + `resolveLandingModels`, attachment types, conversation model (`LandingTurn`, including durable `stopped` terminal state, `TurnPart` variants, optional tool-call argument images, and `RetryPart` inlined), cost/usage types, `LandingAgentSendInput`, and formatting helpers.
...
- `TurnToolBlock` renders validated `http(s)`, root-relative, or supported raster data-URL images inside the expanded Args section. Analyze-image previews come from the client-only attachment payload; screenshot previews come from the server's persisted screenshot URL. Keep image bytes out of durable JSON logs.
