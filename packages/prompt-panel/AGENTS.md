...
- `src/index.ts`: public barrel (`PromptPanel`, `PromptPanelProps`, `readStoredPanelLayout`, `PANEL_WIDTH`, `DEFAULT_PREVIEW_VIEWPORT`, `PREVIEW_VIEWPORTS`, domain model, keyboard-shortcuts, panel-constants types including `PreviewViewport`).
...
- `PanelTheme = 'dark' | 'light' | 'system'` lives in `src/panel-constants.ts`, alongside `PreviewViewport` (`'desktop' | 'mobile' | 'tablet'`), the ordered `PREVIEW_VIEWPORTS` list, and `DEFAULT_PREVIEW_VIEWPORT` (`'desktop'`).
