...
- Design system ported from t3code (`~/Documents/t3code`, apps/web): neutral-black dark palette (`--background` oklch neutral-950, white-alpha `--border`/`--input`/`--muted`/`--accent`), light zinc palette, status tokens (`--info/--success/--warning` + `-foreground`), `dropdown-glass`/`dialog-glass`/`dialog-backdrop-glass` utilities (backdrop-blur surfaces), thin `--app-scrollbar-*` webkit scrollbars, `animate-skeleton`/`animate-status-pulse` keyframes, system font stack (`--font-sans` = -apple-system…, no webfont dep). User overrides on top of the t3code port (keep these): `--primary` stays the app yellow (`oklch(92.4% 0.12 95.746)`, fg `oklch(0.421 0.095 57.708)`) in both themes — NOT t3code blue; corners stay square (`--control-radius: 0`, components use `rounded-none`) — no rounded UI. Neutral `--ring` (not primary-tinted). Component recipes (button/badge/input/textarea/popover/tooltip/dialog/dropdown-menu/command/input-group) otherwise mirror t3code's Base UI class patterns adapted to our Radix primitives (desktop density = their `sm:` values, Base UI `[:hover,[data-pressed]]` → Radix `hover:`/`active:`/`aria-expanded`, glass surfaces).
