...
- The editor preview initializes a sandboxed `srcDoc` iframe that permits forms, modals, popups, same-origin storage, and scripts, then applies routine `html_update` changes in-place with a same-document DOM morph helper rather than replacing `srcDoc`. Script-changing updates rerun scripts inside the existing iframe document instead of refreshing the frame. The client dynamically injects `<base href="about:srcdoc">` into preview HTML before initial `srcDoc` loads and morph targets only so generated `#section` links resolve inside the iframe without changing persisted project HTML. The content and message history are pulled from the server (they are not generated or persisted in the browser). The composer element-selection toggle installs iframe-local capture listeners, resolves hover/click targets with `elementsFromPoint`, blocks generated-page interactions while active, renders an animated pointer-events-none dashed accent overlay box around hovered/selected elements, then attaches the selected element's `outerHTML` plus padded screenshot to the prompt. For `screenshot_request`, the client pulls the latest project HTML from the server, renders it in a temporary same-origin/no-script offscreen iframe at the requested `mobile`/`tablet`/`desktop` viewport, captures the requested selector with `@zumer/snapdom`, adds 8px padding around the element image, and posts the correlated response to `/api/screenshot-responses/:requestId`. Do not reintroduce a full workspace snapshot, client-side message save path, or browser AI SDK tool loop without updating plans and DOX.
- The prompt panel composer action row is ordered as element select, image attach, then model selector. The model selector is a single dropdown with three separated, labeled radio sections — Text, Vision, Image — bound to a `LandingModels` selection (`{ text, vision, image }`). Selections persist immediately via `PATCH /api/projects/:id { textModel, visionModel, imageModel }`; individual saved message turns still show the text model used for that turn. The server currently persists `textModel` to project metadata; `visionModel`/`imageModel` are sent forward-compat and applied per request, so they reset to defaults on reload until the server stores them. Cost formatting is owned by `@workspace/prompt-panel` and displays USD with exactly four digits after the decimal point.
- The prompt panel header action row is ordered as All Projects, panel layout dropdown, settings dropdown, then minimize/maximize. The panel layout dropdown owns left sidebar, right sidebar, and floating choices; its trigger uses the `PanelsTopLeft` icon and its choices render as plain menu items without checkbox/radio indicators. The settings dropdown owns the theme toggle. Header dropdowns stay compact: no visible group label headings; layout/theme menu items use the shared command shortcut treatment; the panel dropdown trigger itself has no keyboard shortcut. The Command/Control Option A shortcut still navigates to `/`. The panel persists only position/layout plus minimized/maximized state in browser localStorage under `landing.promptPanel.position.v1`.
