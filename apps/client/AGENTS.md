...
- The server owns the project's `index.html` file — it is the single source of truth. There is **no `html` SSE event** and **no client PUT**: after each `edit` tool completes, the UI calls `GET /api/projects/:id` and pulls the updated HTML (see `use-landing-page.ts` `refreshHtml` on `edit` done). On editor mount it also pulls the current HTML once.
