...
- The prompt panel header links back to `/` (all projects), exposes the Command/Control Option A shortcut, and persists only the panel position/layout plus minimized/maximized state in browser localStorage under `landing.promptPanel.position.v1`.
