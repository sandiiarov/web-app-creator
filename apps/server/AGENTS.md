...
- `src/index.ts`: custom Node HTTP server, CORS; `/agent`, `/api/models`, `/api/projects*` REST (incl `/api/projects/:id/stop` graceful run stop), `/images/:id`, `/api/projects/:id/images/:file`, `/api/projects/:id/screenshots/:file` routing.
...
- `src/model-catalog.ts`: OpenRouter `/models` catalog proxy for the UI model picker — slim per-1M-token pricing map (input/output/cache-read) via `boundedFetch`, in-process TTL cache (5 min, matches upstream `max-age`), stale-on-error fallback to last good snapshot, `filterModelPricing` scopes to caller-requested supported ids, `resetModelPricingCache` test hook.
