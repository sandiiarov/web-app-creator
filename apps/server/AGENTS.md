...
- `src/model-catalog.ts`: OpenRouter `/models` catalog proxy for the UI model picker — slim per-1M-token pricing map (input/output/cache-read) via `boundedFetch`, in-process TTL cache (5 min, matches upstream `max-age`), stale-on-error fallback to last good snapshot, `resetModelPricingCache` test hook.
