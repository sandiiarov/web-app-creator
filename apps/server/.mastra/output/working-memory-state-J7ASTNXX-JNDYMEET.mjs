...
var Diff = class {
  ...
  addToPath(path, added, removed, oldPosInc, options) {
    ...
    if (last && !options.oneChangePerToken && last.added === added && last.removed === removed) {
      return {
        lastComponent: { added, count: last.count + 1, previousComponent: last.previousComponent, removed },
        oldPos: path.oldPos + oldPosInc
      };
    } else {
      return {
        lastComponent: { added, count: 1, previousComponent: last, removed },
        oldPos: path.oldPos + oldPosInc
      };
    }
  }
  ...
  diffWithOptionsObj(oldTokens, newTokens, options, callback) {
    ...
    const bestPath = [{ lastComponent: void 0, oldPos: -1 }];
    let newPos = this.extractCommon(bestPath[0], newTokens, oldTokens, 0, options);
    if (bestPath[0].oldPos + 1 >= oldLen && newPos + 1 >= newLen) {
      return done(this.buildValues(bestPath[0].lastComponent, newTokens, oldTokens));
    }
    let maxDiagonalToConsider = Infinity, minDiagonalToConsider = -Infinity;
  ...
  extractCommon(basePath, newTokens, oldTokens, diagonalPath, options) {
    const newLen = newTokens.length, oldLen = oldTokens.length;
    let commonCount = 0, oldPos = basePath.oldPos, newPos = oldPos - diagonalPath;
    while (newPos + 1 < newLen && oldPos + 1 < oldLen && this.equals(oldTokens[oldPos + 1], newTokens[newPos + 1], options)) {
      newPos++;
      oldPos++;
      commonCount++;
      if (options.oneChangePerToken) {
        basePath.lastComponent = { added: false, count: 1, previousComponent: basePath.lastComponent, removed: false };
      }
    }
    if (commonCount && !options.oneChangePerToken) {
      basePath.lastComponent = { added: false, count: commonCount, previousComponent: basePath.lastComponent, removed: false };
    }
...
function structuredPatch(oldFileName, newFileName, oldStr, newStr, oldHeader, newHeader, options) {
  ...
  function diffLinesResultToPatch(diff) {
    ...
    const hunks = [];
    let curRange = [], newLine = 1, newRangeStart = 0, oldLine = 1, oldRangeStart = 0;
...
var WorkingMemoryStateProcessor = class {
  ...
  async computeStateSignal(args) {
    ...
    if (!contents) return;
    const cacheKey = stableWorkingMemoryCacheKey({ data: contents, format: template.format });
...
function buildMarkdownDelta(args) {
  const { deltasSinceSnapshot, lastSnapshot, nextContents } = args;
...
export { stableWorkingMemoryCacheKey, WORKING_MEMORY_STATE_ID, WORKING_MEMORY_STATE_PROCESSOR_ID, WorkingMemoryStateProcessor };
