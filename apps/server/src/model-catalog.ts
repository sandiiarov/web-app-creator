...
export type ModelPricingEntry = {
  cacheRead?: number
  /** Total context window in tokens (upstream `context_length`). */
...
export function createModelCatalog({
   ...
   */
  async function getImageModelPricing(
    ids: readonly string[],
    signal?: AbortSignal,
    ...
    await Promise.all(
      [...stale].map(async (id) => {
        try {
          const fetched = await runStandaloneProviderOperation(
            createScope,
            `image-model-pricing:${id}`,
            (operation) =>
              transport.json<ImageModelEndpointsResponse>({
                init: { method: 'GET' },
                label: 'OpenRouter image model pricing',
                operation,
                retry: 'safe',
                url: IMAGE_ENDPOINTS_URL(id),
              }),
            {
          ...
          const entry = parseImageEndpointsPricing(fetched.value)
   ...
   */
  function parseImageEndpointsPricing(
  ...
  function resetModelPricingCache() {
    snapshot = undefined
    inflight = undefined
  ...
  async function refresh(signal?: AbortSignal): Promise<ModelPricingCatalog> {
    try {
      const result = await runStandaloneProviderOperation(
        createScope,
        ...
        { operationTimeoutMs: 15_000, signal },
      )
      if (!result.ok) throw result.error
      const data = parseModelPricing(result.value)
  ...
  return {
    filter: filterModelPricing,
    filterModelPricing,
    get: getModelPricing,
    getImageModelPricing,
    getImages: getImageModelPricing,
    getModelPricing,
