...
const LANDING_TOOL_DEFINITIONS = [
  tool('scrape', 'Use `scrape` when the user gives a reference URL or asks you to match a brand. It returns markdown, links, image URLs, branding, and `imageOcr` — the OCR + visual transcript for all scraped images. Use `imageOcr.text` directly. Prefer relevant URLs from `images` for source-site content such as portraits, logos, screenshots, newsletter art, and video thumbnails; do not hotlink arbitrary image URLs that were not returned by `scrape`. If `images` is empty, say no OCR was possible.', () => createScrapeTool()),
  tool('read', 'Use `read` to inspect the current `/index.html` before making exact edits.', ({ store }) => createReadTool(store)),
  tool('grep', 'Use `grep` to locate exact text or CSS before editing.', ({ store }) => createGrepTool(store)),
  tool('edit', 'Use `edit` to change `/index.html`. Make precise replacements; after every successful edit the preview receives the full updated HTML.', ({ store }) => createEditTool(store)),
  tool('generate_image', 'Use `generate_image` whenever the landing page would benefit from net-new raster imagery or art-directed visual assets (hero art, editorial photos, product scenes, abstract brand visuals, textures, etc.). Prefer `scrape.images` for faithful source-site imagery; generate new imagery when scraped assets are missing, low quality, legally/visually unsuitable, or when a new concept strengthens the page. It returns a hosted URL such as `http://localhost:3001/images/img-1.jpg`; embed that URL directly in `<img src="...">`. Do not use placeholders or pasted image bytes.', ({ baseUrl }) => createGenerateImageTool(baseUrl)),
