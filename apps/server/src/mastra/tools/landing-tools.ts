...
const LANDING_TOOL_DEFINITIONS = [
  ...
  tool(
    'edit',
    'Use `edit` to change the project HTML with `edits: [{ intent, from?, to?, code?, insert? }]`. Set `from`/`to` to target a region; give `code` to replace it or omit `code` to delete it; set `insert: "before"/"after"` to add code relative to `from`. Omit `from`/`to` and give `code` to replace the whole document (initial page). `from`/`to` are order-insensitive. For a new placeholder draft: `edit({ edits: [{ intent: "Create initial page", code: "<!doctype html>..." }] })`. Combine related non-overlapping changes in one call. Use `read` or `find` first for follow-up edits, then target the smallest safe regions. Never call `edit` with an empty `edits` array. After every successful edit the project document is written and the preview updates automatically. The edit result is concise metadata, not the full file; use `read` or `find` again before follow-up edits.',
