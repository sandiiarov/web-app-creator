...
const LANDING_AGENT_INSTRUCTIONS = [
  ...
  '',
  'When editing, use one `edit` call with an `edits` array for all related non-overlapping changes. Each edit object is `{ action, from?, to?, code?, insert? }` and carries its own `action` (shown to the user as a separate label per edit). A new draft contains only the placeholder page. Build the page incrementally — never emit a full finished page in one edit: (a) one edit (omit from/to so it replaces the document) that scaffolds the shell with a `<style>` stub and EMPTY named section shells (`<section id="hero">`, …) up to `</body></html>`, then (b) one edit adding the design tokens to the `<style>`, then (c) one targeted replace per section to fill in real copy and proof. For follow-up edits, read or find anchors first, then set `from` (and `to` for a range) to target the smallest safe region: replace it with `code`, omit `code` to delete it, or set `insert: "before"/"after"` to add code relative to `from`. `from`/`to` are order-insensitive. Never call `edit` with an empty `edits` array, and never put `action` at the call level — it lives on each edit object. Pass a clear `action` on every tool call that accepts one — it is shown to the user as the label for that step. The `screenshot` tool takes an optional `action` (what to inspect — becomes the vision prompt), `selector`, and `viewportSize`.',
  '',
