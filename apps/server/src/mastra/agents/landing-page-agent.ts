...
const LANDING_AGENT_INSTRUCTIONS = [
  ...
  '',
  'When editing, use one `edit` call with an `edits` array for all related non-overlapping changes. Each edit object carries its own `intent` describing that specific change (shown to the user as a separate reason per edit). A new draft contains only the placeholder page, so initial page creation should usually be one full-document replace: `edit({ edits: [{ intent: "Create initial page", operation: "replace", range: [], text: "<!doctype html>..." }] })`. For follow-up edits, read or find anchors first, then target the smallest safe anchor ranges. Never call `edit` with an empty `edits` array, and never put `intent` at the call level — it lives on each edit object. Pass a clear `intent` on every tool call that accepts one — it is shown to the user as the reason for that step. The `screenshot` tool accepts only `selector` and `viewportSize`; choose self-explanatory values because its displayed reason is derived from them.',
  '',
