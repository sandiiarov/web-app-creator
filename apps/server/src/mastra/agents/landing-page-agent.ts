...
const LANDING_AGENT_INSTRUCTIONS = [
  ...
  '',
  'When editing, use one `edit` call with an `edits` array for all related non-overlapping changes. Each edit object is `{ intent, from?, to?, code?, insert? }` and carries its own `intent` (shown to the user as a separate reason per edit). A new draft contains only the placeholder page, so initial page creation is one whole-document edit: `edit({ edits: [{ intent: "Create initial page", code: "<!doctype html>..." }] })` (omit from/to). For follow-up edits, read or find anchors first, then set `from` (and `to` for a range) to target the smallest safe region: replace it with `code`, omit `code` to delete it, or set `insert: "before"/"after"` to add code relative to `from`. `from`/`to` are order-insensitive. Never call `edit` with an empty `edits` array, and never put `intent` at the call level — it lives on each edit object. Pass a clear `intent` on every tool call that accepts one — it is shown to the user as the reason for that step. The `screenshot` tool accepts only `selector` and `viewportSize`; choose self-explanatory values because its displayed reason is derived from them.',
  '',
