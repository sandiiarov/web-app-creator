...
export const designSkill = createSkill({
  ...
  instructions: [
    ...
    '2. Build incrementally — never emit a full finished page in one edit. Use the hashline DSL: each `edit` is `{ action, diff }` where `diff` starts with the `[index.html#TAG]` header copied from your latest `read`/`find`, then `SWAP N.=M:`/`DEL N.=M`/`INS.PRE|POST|HEAD|TAIL N:` ops with `+TEXT` body rows. (a) Scaffold: `read` the placeholder for a #TAG, then one edit (`INS.HEAD:` or a whole-doc `SWAP`) with the document shell, a minimal reset and a `<style>` stub in `<head>`, and EMPTY named section shells in `<body>` (`<section id="hero">`, `<section id="features">`, …) up to `</body></html>` — no section content, keep the diff small. (b) Tokens: `read` the `<style>` lines for a fresh #TAG, then one `SWAP` adding the design tokens (color, type scale, spacing, `border-radius:0`) derived from brand/scrape signals. (c) Fill sections: `read`/`find` each section for its #TAG + line numbers, then one targeted `SWAP` per section with real copy, proof object, and imagery — each edit well under the output limit. Touch only lines your read displayed; ranges cover only changed lines. Use `skill_read` for the compact reference that matches the design problem.',
    '3. Combine related changes in one `edit` diff (multiple hunks) when practical. Each `diff` must start with the `[index.html#TAG]` header from your latest `read`/`find`. Use `SWAP N.=M:` + `+TEXT` to replace lines, `DEL N.=M` to delete, `INS.PRE|POST|HEAD|TAIL N:` to add. Numbers refer to the ORIGINAL file and never shift as hunks apply; touch only lines your read displayed. On a stale-`#TAG` rejection, re-`read` before retrying. Keep ranges small (only changed lines — pure additions use INS, never a widened SWAP).',
