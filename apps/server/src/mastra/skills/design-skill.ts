...
export const designSkill = createSkill({
  ...
  instructions: [
    ...
    '2. For a new placeholder draft, create the first page with one full-document replace edit: `edit({ intent, edits: [{ operation: "replace", range: [], text: "<!doctype html>..." }] })`. For follow-up changes, read the current project HTML with `read` or `find` before edits. Use `skill_read` for the compact reference that matches the design problem.',
    '3. Apply related changes with one batched `edit` call when practical. Every `edit` call must include a non-empty `edits` array. Use anchor ranges from `read`/`find`, keep ranges small, and avoid stale anchors.',
