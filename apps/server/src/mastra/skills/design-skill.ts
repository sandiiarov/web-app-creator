...
export const designSkill = createSkill({
  ...
  instructions: [
    ...
    '2. Build incrementally — never emit a full finished page in one edit. (a) Scaffold: one edit (omit from/to so it replaces the document) with the document shell, a minimal reset and a `<style>` stub in `<head>`, and EMPTY named section shells in `<body>` (`<section id="hero">`, `<section id="features">`, …) up to `</body></html>` — no section content, keep `code` small. (b) Tokens: one edit adding the design tokens (color, type scale, spacing, `border-radius:0`) to the `<style>`, derived from brand/scrape signals. (c) Fill sections: `read`/`find` the section anchors, then one targeted replace per section with real copy, proof object, and imagery — each edit well under the output limit. Use `skill_read` for the compact reference that matches the design problem.',
