...
describe('replayClientMessages (hydration reducer)', () => {
  ...
  const prompt = (turnId: string, extra: Record<string, unknown> = {}) => ({
    dir: 'in' as const,
    model: 'm',
    prompt: 'hi',
    ts: 't0',
  ...
  it('ignores an error with message "stopped" but records other errors', () => {
    const stopped = replayClientMessages([
      prompt('turn-1'),
      out('error', { message: 'stopped' }),
    ])
    expect(stopped[0]?.error).toBeUndefined()

    const errored = replayClientMessages([
      prompt('turn-2'),
      out('tool_call', { id: 'c', state: 'running', tool: 'edit' }),
      out('error', { message: 'boom' }),
    ])
    expect(errored[0]?.error).toBe('boom')
    // A running tool left behind by an error is terminalized to 'error' with
    // the error message as its result.
  ...
  it('keys the turn id on the prompt-in turnId, falling back to turn-N', () => {
    const withId = replayClientMessages([prompt('abc')])
    expect(withId[0]?.id).toBe('abc')

    const withoutId = replayClientMessages([
      { dir: 'in', prompt: 'hi', ts: 't0', type: 'prompt' },
    ])
    expect(withoutId[0]?.id).toBe('turn-1')
  })
  ...
  // Known divergence (plan 006 STOP): the live client reducer appends a
  // 'retry' part on a `retry` SSE event, but the server's hydration reducer
  // intentionally skips `retry` (no turn-structure effect), so retry parts do
  // not survive a reload. Pinned here so a future reconciler sees it.
